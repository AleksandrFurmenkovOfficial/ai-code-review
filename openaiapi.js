...
class OpenAIAPI {
  constructor(apiKey, fileContentGetter, fileCommenter, maxSymbols) {
    this.openaiClient = new openai.OpenAIApi(
    ...
    this.messages = [
      {
        ...
        content:
          ...
          "You speak French, in your comments",
      },
    ];
  }
      ...
      try {
          ...
          functions: [
            {
              name: "getFileContent",
              description:
                "Get the file content to better understand the changes",
              parameters: {
                type: "object",
                properties: {
                  pathToFile: {
                    type: "string",
                    description:
                      "The fully qualified path to file which needed to specify to get the file content.",
                  },
                  startLineNumber: {
                    type: "integer",
            ...
            },
            {
              ...
              description:
                "Add an AI review comment to the line that attracted attention in the review.",
              parameters: {
                type: "object",
                properties: {
                  fileName: {
                  ...
                  lineNumber: {
                    type: "integer",
                    description:
                      "The line number of the file which needed to specify to place comment for a right piece of code.",
                  },
                  reviewCommentFromAIExpert: {
                    type: "string",
                    description:
                      "Your answer, the code review comment (from you) is meant for the user to read and take into account.",
                  },
                },
                required: [
                  "fileName",
        ...
        if (requestToUseFunction) {
          ...
          if (functionToUse.name === "getFileContent") {
            ...
            const requestedFileContent = await this.fileContentGetter(
              args.pathToFile
            );
            this.addFunctionResult(
              "getFileContent",
              this.wrapFileContent(args.pathToFile, requestedFileContent)
            );
            if (this.getUsedSymbols() > this.maxSymbols) {
              const removed = this.messages.pop();
              console.info("Too long, removed:", removed);
              const shortenData = requestedFileContent.substring(
                args.startLineNumber - 20,
                args.endLineNumber + 20
              );
              this.addFunctionResult(
                "getFileContent",
                this.wrapFileContent(args.pathToFile, shortenData)
              );
              if (this.getUsedSymbols() > this.maxSymbols) {
            ...
            this.addUserMsg(
              "Use the addReviewCommentToFileLine to comment lines that necessitates a comment."
            );
            retries = 0;
            continue;
